...
func NewOwnerRepository(
	...
	if database == nil || database.native == nil {
		return nil, errors.New(
			"construct MySQL owner repository: database is nil",
		)
	}
...
func saveOwnerRow(
	...
	if value.New() {
		...
		return assignInsertedID("save owner: insert", result, &value.ID)
	}
	result, err := transaction.ExecContext(
		ctx,
		updateOwnerSQL,
...
func savePet(
	...
	if value.New() {
		...
		if err := assignInsertedID(
			"save owner: insert pet",
			result,
			&value.ID,
		); err != nil {
			return err
...
func saveVisit(
	...
	if value.New() {
		...
		return assignInsertedID(
			"save owner: insert visit",
			result,
			&value.ID,
		)
	}
...
func assignInsertedID(
	operation string,
	result sql.Result,
	target *model.ID,
) error {
	identity, err := result.LastInsertId()
	if err != nil {
		return fmt.Errorf("%s result: %w", operation, err)
	}
	if identity < 1 {
		return fmt.Errorf("%s result: identity is invalid", operation)
	}
	*target = model.ID(identity)
	return nil
}
