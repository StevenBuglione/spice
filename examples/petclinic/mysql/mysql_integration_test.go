...
func TestMySQLPetclinicWorkflow(t *testing.T) {
	...
	if _, err := database.native.ExecContext(
		ctx,
		`DELETE FROM spice_schema_history WHERE version = ?`,
		schemaVersion,
	); err != nil {
		t.Fatalf("delete schema history error = %v", err)
	}
	if err := database.Migrate(ctx); err != nil {
	...
	if _, err := database.native.ExecContext(
		ctx,
		`UPDATE spice_schema_history SET checksum = ? WHERE version = ?`,
		strings.Repeat("0", 64),
		schemaVersion,
	); err != nil {
		t.Fatalf("alter schema history error = %v", err)
	}
	if err := database.Migrate(ctx); err == nil ||
		!strings.Contains(err.Error(), "checksum changed") {
		t.Fatalf("Migrate(drift) error = %v", err)
	}
}
