...
OpenAPI in that package, a tiny `zz_spice_bridge_gen.go` beside `main.go`, and
narrow source-owned shards for compile-time interface assertions. A compatible
legacy marker needs only the internal generated package. The module root
remains an in-memory execution detail and never appears in generated bytes.
...
entrypoint package, file roles, declaration source origins,
