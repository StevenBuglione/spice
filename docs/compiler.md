...
package-main marker to `internal/spicegen/<target>/zz_spice_gen.go`, optional
OpenAPI under `artifacts/`, a tiny `zz_spice_bridge_gen.go` beside `main.go`,
and one mirrored source unit per contributing handwritten file under
`sources/`. Source units own direct provider construction, typed configuration
binding, and compile-time interface assertions. Target-wide graph and lifecycle
coordination remains in the orchestrator rather than being falsely attributed
to an arbitrary source file. A compatible legacy marker needs only the
internal generated package. The module root remains an in-memory execution
detail and never appears in generated bytes.
...
Each plan includes canonical schema-4 JSON ownership metadata with target,
entrypoint package, file roles, primary and related declaration origins, exact
source-to-generated ranges,
...
Schema-3 adjacent source shards are accepted only as migration input and are
removed only when their recorded hash still matches; modified legacy output
fails closed.
