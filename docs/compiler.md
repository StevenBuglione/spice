...
OpenAPI under `artifacts/`, and one mirrored source unit per contributing
handwritten file under `sources/`. Source units own application-marker
metadata, direct provider construction, typed configuration binding, and
compile-time interface assertions. Target-wide graph and lifecycle
coordination remains in the orchestrator rather than being falsely attributed
to an arbitrary source file. No generated Go is written beside handwritten
source. The module root remains an in-memory execution detail and never
appears in generated bytes.
