...
| Core | Starters and dependency alignment | A public strict `spice.starter/v1` SDK and committed `spice.starters/v1` repository selection provide fail-closed compatibility, annotation/feature composition, and generation freshness without dependency-presence activation; an explicit compiler seam validates selected entrypoints in the shared typed program and emits direct constructor calls with cleanup/rollback and provenance hashing; PostgreSQL, OIDC, OAuth2-client, and OpenTelemetry publish constructor manifests, while CLI entrypoint mapping and dependency alignment follow | in-progress |
