...
| Data | PostgreSQL, Redis, MongoDB, Elasticsearch, Cassandra, Neo4j | A reviewed pgx `database/sql` starter provides secure PostgreSQL pool configuration plus real-container transaction, query, advisory-lock, and migration verification; a reviewed go-redis starter provides secure bounded client ownership and a typed JSON `cache.Store` with real-server expiration/cancellation verification; other curated clients remain separate opt-in integrations | integration |
