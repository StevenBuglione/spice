...
| Security | Resource server and client | Opt-in JWT resource servers verify signature, exact issuer/audience, expiry, strict bearer syntax, and bounded role/scope claims before creating principals; HTTPS-only bounded discovery/JWK refresh is available, while opaque introspection and OAuth2 clients follow | in-progress |
