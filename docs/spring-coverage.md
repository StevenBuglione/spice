...
| Security | Resource server and client | Opt-in JWT resource servers verify signature, exact issuer/audience, expiry, strict bearer syntax, and bounded role/scope claims; HTTPS-only discovery/JWK refresh and bounded OAuth2 client-credentials HTTP clients are available, while opaque-token introspection and delegated-user clients follow | in-progress |
