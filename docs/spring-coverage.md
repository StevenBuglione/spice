...
| Core | Starters and dependency alignment | A public strict `spice.starter/v1` SDK and committed `spice.starters/v1` repository selection provide fail-closed compatibility and explicit constructor/annotation activation without dependency-presence semantics; the CLI loads constructor packages as isolated roots in the shared typed program, validates exact provider signatures/graphs, and emits direct calls with cleanup/rollback and provenance hashing; PostgreSQL, OIDC, OAuth2-client, and OpenTelemetry publish constructor manifests, while dependency-version alignment follows | in-progress |
