...
| Data | PostgreSQL, Redis, MongoDB, Elasticsearch, Cassandra, Neo4j | A reviewed pgx `database/sql` starter provides secure PostgreSQL pool configuration plus real-container transaction, query, advisory-lock, and migration verification; other curated client starters remain separate opt-in integrations | integration |
...
| Scheduling | Scheduled and asynchronous work | Bounded asynchronous execution plus compile-time `@schedule.FixedDelay` methods generate exact provider-owned jobs with direct calls, non-overlap, explicit error continuation, deterministic failures, panic containment, graceful drain, module-aware observations, and virtual-time seams; `@async.Execute` now validates exact provider-owned typed methods and immutable submit metadata, while generated submission follows | in-progress |
| Batch | Batch jobs | Job/step abstraction with restart and observability support | planned |
| Cache | Cache abstraction | Generic stores and a bounded concurrent LRU/TTL implementation provide explicit expiration, caller-owned time, key-free observations, and aggregate metrics; `@cache.Cacheable` typed GET declarations compile into fail-closed module-aware exact key/value metadata, configured bounded store construction, and direct hit/miss route logic; distributed stores remain opt-in integrations | available |
