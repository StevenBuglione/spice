...
| Events | Application events | Immutable generic topics provide exact payload types and deterministic in-process delivery; `@event.Topic`/`@event.Listener` declarations now compile into module-aware listener metadata and a synthetic exact publisher provider graph, while direct topic generation follows; bounded transactional messages, a driver-neutral SQL store, atomic enqueue/lease contracts, and at-least-once dispatch are also available | in-progress |
