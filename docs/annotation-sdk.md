...
not enumerate its own index to decide what Spice may inject. When the compiler
has enough provider metadata, accepting the completion also adds the ordinary
Go import and mandatory compile-time assertion in one version-checked edit.
Native Implement Methods and assertion actions remain useful authoring and
offline fallbacks, but they cannot register or select a bean; the next Spice
